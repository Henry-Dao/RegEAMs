%% DESCRIPTION: create the plot in Section 5.3 - HCP data
clear all;  clc
Big_title = 'RegLBA';    Big_title_size = 20;
load('VB_result.mat')
...
linked_parameter_simplified_names = {'$\log\left(c\right)$','$v_{1}^{(\textrm{l})}$','$v_{2}^{(\textrm{l})}$',...
    '$v_{1}^{(\textrm{t})}$','$v_{2}^{(\textrm{t})}$',...
    '$v_{1}^{(\textrm{n})}$','$v_{2}^{(\textrm{n})}$'};
...
for i = 1:model.beta_dim(2)
    ...
    hold on
    x = 1:1:7;
    percentiles = prctile(VB_results.theta_VB.beta_vec(idx(1:7),:),[2.5 50 97.5],2);
    y1 = percentiles(:,2);
    yneg1 = abs(y1 - percentiles(:,1));
    ypos1 = abs(y1 - percentiles(:,3));

    percentiles = prctile(VB_results.theta_VB.beta_vec(idx(8:14),:),[2.5 50 97.5],2);
    y2 = percentiles(:,2);
