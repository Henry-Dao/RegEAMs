function output = prior_density_Hybrid_v5(model,alpha,mu_alpha,Sigma_alpha,a)
%% DESCRIPTION: 
...
% ---------------------- gradient wrt log(a) ---------------------------
    ...
    output.log = -0.5*D_alpha*(J+1)*log(2*pi) - 0.5*(J + v + D_alpha +1)*logdet(Sigma_alpha) -... 
        0.5*trace((alpha-mu_alpha)'*A) - 0.5*logdet(model.prior_par.cov,'chol') - 0.5*(mu_alpha-model.prior_par.mu)'/model.prior_par.cov*(mu_alpha-model.prior_par.mu)+...
