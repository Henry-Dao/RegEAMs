...
clear all; clc
load('Simulated_data_medium.mat')
...
%======================= Set up legend names
legend_names = {'MCMC', 'VB', 'true values'};
...
subplot_count = max_subplot + 1;
vech_Sigma_alpha_true = vech(true_values.Sigma_alpha);
true_values.diagonal_Sigma_alpha = diag(true_values.Sigma_alpha);
...
for i = 1:D_alpha
    ...
    plot(x,y,'LineWidth',line_size);
    xline(true_values.diagonal_Sigma_alpha(i),'LineWidth',line_size);
    hold off
    title(title_names{subplot_count},'Fontsize',18,'Interpreter',"latex")
    if (subplot_count == 1)
        % add a bit space to the figure
