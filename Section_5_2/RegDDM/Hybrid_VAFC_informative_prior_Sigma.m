function output = Hybrid_VAFC_informative_prior_Sigma_v7(model,data,VB_settings)
...
if ~isfield(VB_settings,'LB')
    for t = 1:window
        ...
        % (1) Generate theta  
            ...
            beta_vec = theta_1(D_alpha*J + 1:(D_alpha*J+d_alpha*num_covariates));
            mu_alpha = theta_1((D_alpha*J+d_alpha*num_covariates) + 1:end);
        ...
        % Update learning rate and Lambda
            ...
            if VB_settings.store_lambdas == true && mod(t,VB_settings.lambda_store_period) == 0
                lambda_store{count,1} = lambda;
                count = count + 1;
            end
...
%         theta_VB(:,i) = [ALPHA(:); beta_vec; mu_alpha; vech(C_star); log_a];        
    end 
    output.theta_VB.alpha = ALPHA;
    output.theta_VB.mu_alpha = mu_alpha;
    output.theta_VB.beta_vec = beta_vec;
...
%% Save the output
    if VB_settings.store_lambdas == true
        output.lambda_store = lambda_store(1:count-1,1);
    end
